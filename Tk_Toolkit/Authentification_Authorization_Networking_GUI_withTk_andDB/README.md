...
- If the server gets "[USER-UPDATE-DATA]{Username: "...", Password: "...", UpdateProperty : "...", TYPE: "..."}" type of format ( so if it starts with [USER-UPDATE-DATA], we will eval the dict after checking that ), check if the given username and password are valid. If they are valid, update the property and then send the [USER-UPDATE-SUCCESS] back to the client. If the user wasn't found, then don't update anything and send [USER-UPDATE-ERROR] back to the client.
